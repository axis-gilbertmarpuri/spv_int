...
//     $response = $client->get("/api/v1/menus", ['cookies' => $cookieJar]);
//     $plantJsonDecoded = json_decode($response->getBody(), true);
//     $plants = [];
//     $plantsNonG = [];
//     echo("<pre>");
//     var_dump($plantJsonDecoded);
//     echo("<hr>");
...
//     //     }
//     // }
//     // foreach ($plantsNonG as $plants) {
//     //     var_dump($plants);
//     //     echo("<br>");
//     // }
//     $app->render('testing.phtml', ['plants' => $plants]);
//     return;
// });
...
$app->get('/test/login', function () use($app){
...
if (isset($_GET['username'], $_GET['password'])) {
    ...
    $app->render('test2.phtml');
});

$app->post('/test/new', function () use($app){
    //$request = $app->request->post();
    $uname = $_GET['username'];
    $pass = $_GET['password'];
    echo ($uname);
...
$app->get('/test(/:stat)', function () use ($app) {
    if (isset($_GET['username'], $_GET['password'])) {
        ...
        if ($username == "test" && $password == "A1X2I3S4") {
            ...
            $app->render('test.phtml', ['plants' => $plants, 'plantsNonG' => $plantsNonG]);
        }
        else {
